...
uint64_t c_shellcode::find_codecave(size_t function_size, const std::string& process_name, const std::string& module)
{
	...
	{
		LOG("[!] Could not retrieve sections #1 for '%s'", module.c_str());
		return 0;
	}
	...
	{
		LOG("[!] Could not retrieve sections #2 for '%s'", module);
		return 0;
	}
	...
	if (!codecave)
	{
		LOG("[!] Could not find a code cave for '%s'", module.c_str());
		return 0;
	}
	...
	{
		LOG("[!] Could not read codecave for '%s'", module.c_str());
		return 0;
	}
...
bool c_shellcode::call_function(void* hook, void* function, const std::string& process_name)
{
	...
	*reinterpret_cast<uint64_t*>(jmp_bytes + 6) = reinterpret_cast<uintptr_t>(hook);
	...
	{
		LOG("[!] 1 Failed to write memory at 0x%p", function);
		return false;
	}
	...
	{
		LOG("[!] 2 Failed to write memory at 0x%p", function);
		return false;
	}
